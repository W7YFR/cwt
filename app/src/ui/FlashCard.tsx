 ...
 * routing that through a re-render would repaint the controls, the chart's
 * props and both report tables ten times a second to change one digit. The
 * elements are written to directly, which is the same reason the chart keeps
 * its playhead out of React.
 *
 * It does not light up. An earlier version strobed on every beat, with a dial
 * for how far ahead of it to fire. A bright flash several times a second is a
 * hazard for anyone photosensitive, and it was buying very little: what the
 * card is for is reading what comes next, and a reference you glance at does
 * not need to demand the glance.
 */
...
export function FlashCard({
  ...
  useEffect(() => {
    ...
    const set = (ch: string, time: string) => {
    ...
    const tick = () => {
      const t = elapsed();
      // The next character not yet due: what you are about to send.
      let i = 0;
      ...
      if (next) {
        ...
        set(next.char, `${Math.max(until, 0).toFixed(1)}s`);
      } else {
        set(NOTHING, "—");
      }
  ...
  return (
    ...
    <section className="flashcard" data-testid="flashcard">
